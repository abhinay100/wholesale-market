       ...
       int _selectedIndex;
        ...
        connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["MarketConnectionString"].ToString();
            ...
            try
            {
                ...
                if (_selectedIndex == 0)
                {
                    command.CommandText = " select * from ProductRegistration";
                }
                else if (_selectedIndex == 1)
                {
                    if (comboBox3.Text.Trim() == string.Empty)
                    {
                        MessageBox.Show("Please selsect Item name");
                        return;
                    }

                    command.CommandText = " select * from ProductRegistration  where Name_of_the_product='" + comboBox3.Text + "'";
                }
